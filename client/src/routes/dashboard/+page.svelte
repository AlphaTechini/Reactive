...
<div class="space-y-6" role="main" aria-label="Dashboard">
	<div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg p-6 text-white"><div class="flex items-center justify-between"><div><h1 class="text-2xl font-bold mb-2">Portfolio Dashboard</h1><p class="text-blue-100">Address: <span class="font-mono">{$walletAddress}</span></p></div><div class="text-right"><div class="text-3xl font-bold">{#if $portfolioLoading}<span class="animate-pulse">Loading...</span>{:else}{parseFloat($portfolioBalance||'0').toFixed(4)} REACT{/if}</div><div class="flex items-center justify-end mt-1"><span class="text-sm">Portfolio Balance</span><div class="ml-4"><button class="px-3 py-1 bg-white text-blue-600 rounded" on:click={()=> depositOpen=true}>Deposit</button></div></div></div></div></div>
	...
	<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
		...
		<div class="space-y-6"><PendingTransactions /><ActiveSettings /><QuickActions /><RiskManagementUI {selectedToken} compact={true} /><AllocationManagementUI currentPortfolioValue={parseFloat($portfolioBalance || '0')} compact={true} /><StrategyDrawer {selectedToken} automationAddress={null} /></div>
	</div>
