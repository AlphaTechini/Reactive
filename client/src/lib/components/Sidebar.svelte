<script>
  ...
  import { enhancedPriceService, pricesStore } from '$lib/priceService.js';
  
  export let open=false;
  
  // Token categories with enhanced data structure
  const tokenCategories = {
    btc: [
      { symbol: 'BTC', name: 'Bitcoin', address: '0x1111111111111111111111111111111111111111' },
      { symbol: 'WBTC', name: 'Wrapped Bitcoin', address: '0x1111111111111111111111111111111111111111' }
    ],
    altcoins: [
      { symbol: 'ETH', name: 'Ethereum', address: '0x2222222222222222222222222222222222222222' },
      { symbol: 'LINK', name: 'Chainlink', address: '0x3333333333333333333333333333333333333333' },
      { symbol: 'ADA', name: 'Cardano', address: '0x4444444444444444444444444444444444444444' },
      { symbol: 'DOT', name: 'Polkadot', address: '0x5555555555555555555555555555555555555555' },
      { symbol: 'SOL', name: 'Solana', address: '0x6666666666666666666666666666666666666666' },
      { symbol: 'UNI', name: 'Uniswap', address: '0x7777777777777777777777777777777777777777' }
    ],
    stablecoins: [
      { symbol: 'USDC', name: 'USD Coin', address: '0x8888888888888888888888888888888888888888' },
      { symbol: 'USDT', name: 'Tether', address: '0x9999999999999999999999999999999999999999' }
    ],
    reactive: [
      { symbol: 'REACT', name: 'Reactive Token', address: '0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' }
    ]
  };
  
  let selectedToken = null;
  let query = '';
  let expandedCategories = { btc: true, altcoins: true, stablecoins: false, reactive: true };
  let priceData = {};
  let unsubscribe;
  
  // Subscribe to price updates
  onMount(() => {
    unsubscribe = pricesStore.subscribe(prices => {
      priceData = prices;
    });
  });
  ...
  function formatPrice(price) { 
    return enhancedPriceService.formatPrice(price, 4);
  }
  
  function formatChange(change) { 
    return enhancedPriceService.formatChange(change);
  }
