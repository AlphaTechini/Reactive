<script>
  ...
  let chartContainer; let plotly; let isLoading = true; let candleData = []; let error = null;   let selectedToken = { symbol: 'mBTC', address: '0x1111111111111111111111111111111111111111' };
  ...
  async function loadCandles(){ if(!selectedToken?.address){ candleData=generateSyntheticCandles(); return; } error=null; try{ candleData = await uniswapPriceFeed.getCandles(selectedToken.address, timeframeMapper(timeframe)); }catch(err){ console.error('Failed to load candles:',err); error=err.message; candleData=generateSyntheticCandles(); } }
  async function refreshLatest(){ if(!candleData.length || !selectedToken?.address) return; try{ const latestPrice = await uniswapPriceFeed.getLatestPrice(selectedToken.address); if(latestPrice){ candleData[candleData.length-1].close=latestPrice; updateChart(); sendTick(latestPrice);} }catch(e){ console.warn('Latest price refresh failed', e.message); } }
  ...
  function createChart(){ if(!plotly||!chartContainer||!candleData.length) return; const x=candleData.map(c=>new Date(c.time)); const open=candleData.map(c=>c.open); const high=candleData.map(c=>c.high); const low=candleData.map(c=>c.low); const close=candleData.map(c=>c.close); const increasingColor='#10B981'; const decreasingColor='#EF4444'; const candleTrace={ x, open, high, low, close, type:'candlestick', name:selectedToken?.symbol||'Price', increasing:{ line:{ color:increasingColor } }, decreasing:{ line:{ color:decreasingColor } }, hovertemplate:'<b>%{fullData.name}</b><br>Open: $%{open:.4f}<br>High: $%{high:.4f}<br>Low: $%{low:.4f}<br>Close: $%{close:.4f}<extra></extra>'}; const layout={ title:{ text:`${selectedToken?.symbol||'Token'} (${timeframe})`, font:{ size:15, color:'#374151'}}, xaxis:{ title:'Time', showgrid:true, gridcolor:'rgba(156,163,175,0.2)', type:'date'}, yaxis:{ title:'Price (USD)', showgrid:true, gridcolor:'rgba(156,163,175,0.2)', tickprefix:'$'}, margin:{ t:40, r:20, b:40, l:60}, plot_bgcolor:'rgba(0,0,0,0)', paper_bgcolor:'rgba(0,0,0,0)', font:{ family:'Inter, sans-serif', size:12, color:'#6B7280'}, showlegend:false, dragmode:'pan'}; const config={ responsive:true, displayModeBar:true, scrollZoom:true }; plotly.newPlot(chartContainer,[candleTrace],layout,config); }
  function updateChart(){ if(!plotly||!chartContainer||!candleData.length){ createChart(); return;} const x=candleData.map(c=>new Date(c.time)); const open=candleData.map(c=>c.open); const high=candleData.map(c=>c.high); const low=candleData.map(c=>c.low); const close=candleData.map(c=>c.close); plotly.restyle(chartContainer,{ x:[x], open:[open], high:[high], low:[low], close:[close] },[0]); plotly.relayout(chartContainer,{ 'title.text': `${selectedToken?.symbol||'Token'} (${timeframe})` }); }
...
<div class="relative w-full h-full">
  ...
  {:else}
    <div class="absolute inset-0 overflow-hidden"><div class="absolute inset-0 transition-opacity duration-150" class:opacity-0={updatingTimeframe}><div bind:this={chartContainer} class="w-full h-full"></div></div>{#if updatingTimeframe}<div class="absolute inset-0 flex items-center justify-center bg-white/60 dark:bg-gray-900/40 backdrop-blur-sm"><div class="flex flex-col items-center gap-3"><div class="h-6 w-32 rounded bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700 animate-pulse" /><div class="h-4 w-48 rounded bg-gradient-to-r from-gray-200 via-gray-300 to-gray-200 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700 animate-pulse" /></div></div>{/if}</div>
  {/if}
