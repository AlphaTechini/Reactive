...
<script>
  ...
  function formatEventDetails(type, data) {
    switch (type) {
      ...
      case 'PortfolioRebalanced':
        const tokenList = data.tokens.map((token, i) => `${token}: ${data.allocations[i]}%`).join(', ');
  ...
  function toggleDetails(eventId) {
    showDetails[eventId] = !showDetails[eventId];
    showDetails = { ...showDetails };
  }
  
  function openEtherscan(hash) {
    // Replace with appropriate explorer URL for Reactive Network
    const explorerUrl = `https://explorer.reactive.network/tx/${hash}`;
  ...
  onMount(() => {
    ...
    const unsubscribeWallet = walletStore.subscribe(async (wallet) => {
      if (wallet.isConnected && !isSubscribed) {
        try {
          ...
          contractService.subscribeToEvents({
            onStopLossSet: (data) => addEvent('StopLossSet', data),
            onTakeProfitSet: (data) => addEvent('TakeProfitSet', data),
            onPanicModeTriggered: (data) => addEvent('PanicModeTriggered', data),
            onPanicModeDeactivated: (data) => addEvent('PanicModeDeactivated', data),
            onPortfolioRebalanced: (data) => addEvent('PortfolioRebalanced', data)
          });
          ...
          toast.error('Failed to connect to contract events');
        }
      ...
      <div class="flex items-center">
        <div class="w-8 h-8 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center mr-3">
          <svg class="w-4 h-4 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-5 5v-5zM4 8h14M4 12h8M4 16h5" />
          </svg>
        </div>
        <div>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Contract Events</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {isSubscribed ? 'Monitoring live events' : 'Connect wallet to monitor events'}
          </p>
        </div>
      </div>
    ...
    {#if events.length === 0}
      <div class="px-6 py-8 text-center">
        ...
        <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">
          {isSubscribed ? 'Events will appear here as they occur' : 'Connect your wallet to see contract events'}
        </p>
      </div>
    {:else}
        ...
        {#each events as event (event.id)}
          <div class="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center">
                  <span class="text-lg mr-2">{event.icon}</span>
                  <div>
                ...
                {#if showDetails[event.id]}
                  <div class="mt-3 pl-7">
                    <p class="text-sm text-gray-600 dark:text-gray-300 mb-3">
                      {formatEventDetails(event.type, event.data)}
                    </p>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-xs">
                      <div class="grid grid-cols-1 gap-2">
                        <div class="flex justify-between">
                          ...
                          <button
                            on:click={() => openEtherscan(event.data.transactionHash)}
                            class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-mono"
                          >
                            {truncateHash(event.data.transactionHash)}
                          </button>
                        </div>
              ...
              <div class="flex items-center ml-4">
                ...
                <button
                  on:click={() => toggleDetails(event.id)}
                  class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"
                >
                  <svg 
                    class="w-4 h-4 transition-transform duration-200"
                    class:rotate-180={showDetails[event.id]}
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
